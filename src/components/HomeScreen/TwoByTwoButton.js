...
export default function TwoByTwoButton({ title, onPress, color = 'white', imageSource }) { // 기본값 설정
    return (
        <TouchableOpacity style={[styles.button, { backgroundColor: color }]} onPress={onPress}>
            <View style={styles.titleContainer}>
                <Text allowFontScaling={false} style={textStyles.title32Bold}>{title}</Text>
            </View>
...
const styles = StyleSheet.create({
    ...
    image: {
        ...
        right: 8,
        bottom: 8,
        width: adjustedWidth * (2 / 5), // 이미지 너비
        height: adjustedWidth * (2 / 5), // 이미지 높이
